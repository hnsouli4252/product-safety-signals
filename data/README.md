...
Live on-demand retrieval uses Google News RSS, Bing News RSS, Yahoo News RSS, Yahoo.com News Search, and GDELT DOC 2.0 behind the Site’s `/api/news-search` endpoint. Each provider is isolated so one failure does not suppress the others; successful responses are cached for 15 minutes. The endpoint deduplicates URLs, fetches the highest-ranked pages first, applies the same classifiers, clusters incidents, and returns explicit provider health and human-review warnings. Every candidate retains a provider label so the interface shows which live source supplied it.

Version 2.1 adds guarded fuzzy and semantic action matching. Product-language concepts connect variants such as “toy kitchen,” “play kitchen,” and “kitchen playset,” while hazard concepts connect trapped/stuck/caught, entrapment, strangulation, suffocation, and asphyxia. Exact models, brands, and distinctive product-name words remain the identity anchors. Category-and-hazard similarity without an identity anchor is capped at 68%, labeled “related—review only,” and excluded from linked-action and lead-time metrics. Explicit brand, model, or product-type conflicts reject a match even when the injury language is similar.

The Florida kitchen-playset death is the regression case for this boundary. The app now surfaces CPSC recall 25-415 as a related action because both records involve a children’s play kitchen and fatal entrapment/asphyxia. It does not link the recall: police reported no identifying product markings, visual reporting only suggested an Anatex Country Living Kitchen, and the KidKraft recall describes clothing caught on hooks rather than a head trapped in an opening. The user-visible methodology page explains sources, retrieval cadence, query construction, scoring, matching tiers, cross-checks, clustering, freshness, failure modes, and how to inspect the supporting evidence.
